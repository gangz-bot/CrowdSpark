...
//     // Fetch campaign to get views & followers
...
//       withdrawable: totalDonations  // change logic if you deduct fees etc
//     });
//   } catch (err) {
//     console.error(err);
...
router.get('/:id/stats', async (req, res) => {
  try {
    ...
    const withdrawable = Math.floor(totalDonations * 0.98);

    // Get campaign for views & duration
    const campaign = await Campaign.findById(campaignId).lean();

    // Days left: createdAt + duration (duration in days)
    let daysLeft = 0;
    if (campaign) {
...
// Get donations over time for a campaign
